...
Each Composite ML-DSA algorithm has a single pre-hash algorithm associated with it.
{{sec-signerinfo}} specifies that the SignerInfo's digestAlgorithm field MUST contain this pre-hash algorithm.
If the algorithm in the digestAlgorithm field is different than the pre-hash algorithm of the Composite ML-DSA algorithm used for signing, this implies that the signer may have passed an incorrect digest value to the Composite ML-DSA signing algorithm.
Any resulting signature would therefore not be a valid Composite ML-DSA signature on the data being signed.
If the digestAlgorithm and pre-hash algorithm don't match, the verifier SHOULD reject the message as invalid CMS, but for backwards-compatibility or interoperability reasons MAY instead verify the signature using a digest generated by the Composite ML-DSA pre-hash algorithm.
In general, this is an issue with CMS, where a SignerInfo's digestAlgorithm field might not correspond to the digest required by the SignerInfo's signatureAlgorithm field.
